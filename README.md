...
|[#57](https://github.com/KristianColville1/fine-chop/issues/57)|User should be able to sign up and receive a confirmation email to confirm email address|User enters their details and then it throws a 500 internal serve error|The receiver was insufficiently built and the instance not saved properly. I adapted the solution for the current project and the details save correctly and users can now create profiles on finechop |
|[#58](https://github.com/KristianColville1/fine-chop/issues/58)|The user enters stuff into the cart and then completes the purchase and receives a confirmation email with their order details|User completes purchase and it only alerts the user in the front end that the purchase was successful| The issue was on stripe itself, I spent a lot of time debugging my code and making it conform to the material thought in the course. The api version I was using was incorrect for the webhook on Stripe. I created another webhook and carefully added the November version of the API and testing payments on the live app worked|
