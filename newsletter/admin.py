...
class NewsletterAdmin(admin.ModelAdmin):
    list_display = ('title', 'admin_subscriptions', 'admin_messages',
                    'admin_submissions')
    ...
    def _admin_url(self, obj, model, text):
        ...
        return format_html('<a href="{}?newsletter__id={}">{}</a>', url,
                           obj.id, text)
...
class SubmissionAdmin(NewsletterAdminLinkMixin, ExtendibleModelAdminMixin,
    ...
    list_display = ('admin_message', 'admin_newsletter', 'admin_publish_date',
                    'publish', 'admin_status_text', 'admin_status')
...
if (newsletter_settings.RICHTEXT_WIDGET
    ...
    try:
        ...
        logger.warning('Error importing ImperaviStackedInlineAdmin. '
...
class MessageAdmin(NewsletterAdminLinkMixin, ExtendibleModelAdminMixin,
                   admin.ModelAdmin):
    save_as = True
    list_display = ('admin_title', 'admin_newsletter', 'admin_preview',
    ...
    def admin_preview(self, obj):
        url = reverse('admin:' + self._view_name('preview'),
                      args=(obj.id, ),
                      current_app=self.admin_site.name)
    ...
    def preview_html(self, request, object_id):
        ...
        if not message.html_template:
            raise Http404(
                _('No HTML template associated with the newsletter this '
                  'message belongs to.'))
    ...
    def preview_text(self, request, object_id):
        ...
        return HttpResponse(message.text_template.render(c),
...
class SubscriptionAdmin(NewsletterAdminLinkMixin, ExtendibleModelAdminMixin,
    ...
    list_display = ('name', 'email', 'admin_newsletter',
                    'admin_subscribe_date', 'admin_unsubscribe_date',
                    'admin_status_text', 'admin_status')
    list_display_links = ('name', 'email')
    list_filter = ('newsletter', 'subscribed', 'unsubscribed',
                   'subscribe_date')
    search_fields = ('name_field', 'email_field', 'user__first_name',
    ...
    def make_subscribed(self, request, queryset):
        ...
        self.message_user(
            request,
            ungettext("%d user has been successfully subscribed.",
    ...
    def make_unsubscribed(self, request, queryset):
        ...
        self.message_user(
            request,
            ungettext("%d user has been successfully unsubscribed.",
    ...
    def get_urls(self):
        ...
        if HAS_CBV_JSCAT:
            ...
            my_urls.append(
                url(r'^jsi18n/$',
                    javascript_catalog, {'packages': ('newsletter', )},
                    name='newsletter_js18n'))
